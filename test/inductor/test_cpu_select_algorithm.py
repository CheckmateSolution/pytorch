...
"""
        ...
        def micro_gemm_define(A, B, C, trans_b):
            ...
            from torch._inductor.codegen.cpp_gemm_template import CppGemmTemplate, CppTemplateKernel, parallel_num_threads
            ...
            extra_config = namedtuple("extra_config", ["internal_block_m", "internal_block_n", "trans_b", "expand_n"])
        ...
        shapes = [(1, 64, 128), (31, 63, 61), (32, 64, 64)]
        for trans_b in [False]:
            for accum in [True, False]:
                for shape in shapes:
                    ...
                    TEST_GEMM_TEMPLATE = TEST_GEMM_TEMPLATE.replace("{{input_type}}", dtype_cpp[dtype])
                    GEMM_SOURCE_CODE = TEST_GEMM_TEMPLATE.replace("{{GEMM_DEFINE}}", micro_gemm)
                    GEMM_SOURCE_CODE = GEMM_SOURCE_CODE.replace("{{GEMM_CALL}}", micro_gemm_call)

                    DLL = CppCodeCache.load(GEMM_SOURCE_CODE, device_type="cpu")
                    run_method = getattr(DLL, 'cpp_micro_gemm')
                    inputs = []
                    inputs.append(A)
                    inputs.append(B)
                    inputs.append(C)
                    ...
                    TEST_REF_TEMPLATE = TEST_REF_TEMPLATE.replace("{{input_type}}", dtype_cpp[dtype])
                    REF_GEMM_SOURCE_CODE = TEST_REF_TEMPLATE.replace("{{REF_CALL}}", ref_gemm_call)
                    DLL = CppCodeCache.load(REF_GEMM_SOURCE_CODE, device_type="cpu")
                    run_method = getattr(DLL, 'cpp_ref_gemm')
                    inputs = []
                    inputs.append(A)
                    inputs.append(B)
                    inputs.append(C_ref)
